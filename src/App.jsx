  ...
  for (let i = 0; i < 5; i++) {
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(`HTTP ${res.status}: ${errData?.error?.message || 'Error desconocido de API'}`);
      }
...
export default function App() {
  ...
  const extractFormFromAI = async (prompt, inlineData = null) => {
    ...
    } catch (err) { alert(`Error IA: ${err.message}. Revisa tu Llave API o tu documento.`); } 
  ...
  const handlePdfUploadForEpicrisis = (e) => {
    ...
    reader.onloadend = async () => {
      try {
        ...
        alert(`Error IA al leer documento: ${err.message}. Verifica tu Llave API o el formato del archivo.`);
      } finally {
  ...
  const handleGenerateReport = async (type) => { 
    ...
    try { setReportContent(await generateTextWithRetry(appConfig.apiKey, prompt)); } catch (e) { setReportContent(`Error de conexión IA: ${e.message}`); } finally { setIsGeneratingReport(false); }
  };
  ...
  const handleGenerateCaseSummary = async () => {
    ...
    try { setCaseSummary(await generateTextWithRetry(appConfig.apiKey, prompt)); } catch (e) { setCaseSummary(`Error IA: ${e.message}`); } finally { setIsGeneratingCaseSummary(false); }
  };
