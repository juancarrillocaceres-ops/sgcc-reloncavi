...
const generateTextWithRetry = async (apiKey, prompt, sys = "", inlineData = null) => {
  ...
  for (let i = 0; i < 5; i++) {
    try {
      // Cambiamos al modelo gemini-1.5-flash que es el más estable y no da error 404
      ...
      if (!res.ok) {
        ...
        if (res.status === 429) throw new Error("Límite de consultas (Error 429). Espera 1 minuto e intenta de nuevo.");
        throw new Error(`API Error ${res.status}: ${errData.error?.message || 'Desconocido'}`);
      }
      const data = await res.json();
      if (data.candidates?.[0]?.content?.parts?.[0]?.text) return data.candidates[0].content.parts[0].text;
      throw new Error("Respuesta vacía");
    } catch (e) {
      // Retornar el error amigable inmediatamente si es de cuota (429) o de API
